"""
Tiled Matrix Multiply Microkernel (MMT4D style)
================================================

Computes C[M, N] = A[M, K] @ B[K, N]  (with FP8 inputs, BF16 accumulation)

This implements the core idea from IREE's mmt4d approach: the full matmul
is decomposed into hardware-native tiles, and data is pre-arranged in memory
in a tiled (4D) layout so every DMA load fetches a contiguous tile.

Hardware Natural Tile (from NPU spec):
--------------------------------------
    M0 = 64   (MT: MRF depth, rows per matrix register)
    K0 = 32   (DL: inner-product tree depth / systolic array height)
    N0 = 16   (NL: number of trees / systolic array width)

One `matmul.mxu0` instruction computes:
    C_tile[M0, N0] += A_tile[M0, K0] @ B_tile[K0, N0]
    i.e.  [64, 16] += [64, 32]  @  [32, 16]
    FP8 inputs → BF16 accumulation, takes MT=64 cycles.

Memory Layout (MMT4D / pre-tiled):
-----------------------------------
    A is stored as A[M/M0, K/K0, M0, K0] — each (M0×K0) FP8 tile is 2048 bytes
    B is stored as B[K/K0, N/N0, K0, N0] — each (K0×N0) FP8 tile is  512 bytes
    C is stored as C[M/M0, N/N0, M0, N0] — each (M0×N0) BF16 tile is 2048 bytes

    The compiler (your colleague's job) handles re-laying out the original
    2D matrices into this tiled format before the kernel runs. The kernel
    just reads contiguous tiles via DMA.

Loop Structure (output-stationary):
------------------------------------
    for m_tile in range(M // M0):
        for n_tile in range(N // N0):
            clear accumulator
            for k_tile in range(K // K0):
                DMA load weight B_tile[k_tile, n_tile] -> WB[0]
                DMA load activation A_tile[m_tile, k_tile] -> MRF[act_reg]
                matmul.mxu0: acc_reg += act_reg @ WB[0]
            DMA store acc_reg -> C_tile[m_tile, n_tile]

    Output-stationary order is chosen so we fully accumulate across the K
    dimension in-register before writing out, minimizing output traffic.

Register Allocation:
---------------------
    MRF 0:  accumulator (output tile, BF16 64×16)
    MRF 1:  activation tile (input, FP8-viewed as 64×32)
    MRF 2:  scratch (used only if double-buffering activation loads)

    WB 0:   weight tile (FP8, 32×16)
    WB 1:   (available for weight double-buffering in future optimization)

Usage:
------
    # For a 128×48 = 128×64 @ 64×48 matmul:
    program = make_tiled_matmul_program(M=128, N=48, K=64)

    # Or use the default example:
    prog = TiledMatmulProgram  # 128×32 = 128×64 @ 64×32
"""
