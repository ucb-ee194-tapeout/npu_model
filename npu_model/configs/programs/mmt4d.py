...
# ============================================================================
# Additional size variants — each exercises a different tile-count combination.
# These are picked up automatically by test_programs.py and verified against
# their tile-accurate golden references.
# ============================================================================

def _make_program_class(name: str, M: int, N: int, K: int, doc: str) -> type:
    instrs, mem_regions, _, golden = make_tiled_matmul_program(M=M, N=N, K=K)
    return type(
        name,
        (Program,),
        {
...
SingleTileMatmulProgram = _make_program_class(
    "SingleTileMatmulProgram",
    M=M0, N=N0, K=K0,
    doc="Single-tile matmul (1×1×1): C[64,16] = A[64,32] @ B[32,16].",
)

TwoMTilesMatmulProgram = _make_program_class(
    "TwoMTilesMatmulProgram",
    M=2 * M0, N=N0, K=K0,
    doc="Two M-tiles, one N-tile, one K-tile (2×1×1): C[128,16] = A[128,32] @ B[32,16].",
)

TwoNTilesMatmulProgram = _make_program_class(
    "TwoNTilesMatmulProgram",
    M=M0, N=2 * N0, K=K0,
    doc="One M-tile, two N-tiles, one K-tile (1×2×1): C[64,32] = A[64,32] @ B[32,32].",
)

TwoKTilesMatmulProgram = _make_program_class(
    "TwoKTilesMatmulProgram",
    M=M0, N=N0, K=2 * K0,
    doc="One M-tile, one N-tile, two K-tiles (1×1×2): C[64,16] = A[64,64] @ B[64,16].",
)

NonSquareMatmulProgram = _make_program_class(
    "NonSquareMatmulProgram",
    M=2 * M0, N=3 * N0, K=2 * K0,
    doc="Non-square multi-tile (2×3×2): C[128,48] = A[128,64] @ B[64,48].",
)
