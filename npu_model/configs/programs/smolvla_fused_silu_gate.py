...
class SmolVLAFusedSiluGateProgram(Program):
    ...
    Optimization: vli.all constants initialized during DMA load window (~1028cy).
    ...
    instructions: List[Instruction[Any]] = [
        # Scalar setup
        Instruction(mnemonic="lui", args=ScalarArgs(rd=1, imm=0x2)),  # VMEM_X    = 0x2000
        Instruction(mnemonic="lui", args=ScalarArgs(rd=2, imm=0x3)),  # VMEM_OUT  = 0x3000
        ...
        Instruction(mnemonic="addi", args=ScalarArgs(rd=4, rs1=4, imm=-2048)),  # DRAM_OUT = 0x0800
        ...
        Instruction(mnemonic="delay", args=ScalarArgs(imm=65)),
        Instruction(mnemonic="dma.wait.ch<N>", args=DmaArgs(channel=0)),
        # Load both halves of x into (m0, m1) now that DMA finished.
        ...
        Instruction(mnemonic="vmul.bf16", args=VectorArgs(vd=14, vs1=12, vs2=0)), # m14/m15 = silu(x)
        Instruction(mnemonic="delay", args=ScalarArgs(imm=66)),
        # Store: MRF → VMEM → DRAM
