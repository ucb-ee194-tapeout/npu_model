...
def test_zero_init_is_default_when_randomization_is_disabled() -> None:
    ...
    assert state.read_dram(0, 16).sum().item() == 0
    assert state.read_vmem(0, 0, 16).sum().item() == 0
...
def test_random_init_is_deterministic_with_seed_42() -> None:
    ...
    assert torch.equal(a.read_dram(0, 64), b.read_dram(0, 64))
    assert torch.equal(a.read_vmem(0, 0, 64), b.read_vmem(0, 0, 64))
    assert a.xrf == b.xrf
    assert a.csrf == b.csrf
    assert a.read_dram(0, 64).sum().item() != 0
    assert a.read_vmem(0, 0, 64).sum().item() != 0
